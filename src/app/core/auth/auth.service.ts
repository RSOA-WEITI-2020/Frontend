...
export class AuthService {
  ...
  register(email: string, password: string, firstName: string, lastName: string, address: string): Observable<boolean> {
    return this.authAPI.register(email, password, firstName, lastName, address).pipe(
      switchMap(() => this.authAPI.signIn(email, password)),
      tap((tokenBundle: TokenBundle) => this.setTokenBundle(tokenBundle)),
      catchError((err) => {
        this.setTokenBundle(null);
        return throwError(err);
